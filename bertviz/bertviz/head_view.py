    ...
    """Render head view
        ...
        Args:
            For self-attention models:
                attention: list of ``np.ndarray``(one for each layer) of shape
                    ``(num_heads, sequence_length, sequence_length)``
                tokens: list of tokens
                sentence_b_start: index of first wordpiece in sentence B if input text is sentence pair (optional)
            For encoder-decoder models:
                encoder_attention: list of ``np.ndarray``(one for each layer) of shape
                    ``(num_heads, encoder_sequence_length, encoder_sequence_length)``
                decoder_attention: list of ``np.ndarray``(one for each layer) of shape
                    ``(num_heads, decoder_sequence_length, decoder_sequence_length)``
                cross_attention: list of ``np.ndarray``(one for each layer) of shape
                    ``(num_heads, decoder_sequence_length, encoder_sequence_length)``
    ...
    if attention is not None:
        ...
        if sentence_b_start is None:
            attn_data.append(
                {
                    'name': None,
                    'attn': [a.tolist() for a in attention],  # Convert numpy arrays to lists
                    'left_text': tokens,
                    'right_text': tokens
                }
            )
            ...
            attn_data.append(
                {
                    ...
                    'attn': [a[:, slice_a, slice_a].tolist() for a in attention],  # Convert numpy arrays to lists
            ...
            attn_data.append(
                {
                    ...
                    'attn': [a[:, slice_b, slice_b].tolist() for a in attention],  # Convert numpy arrays to lists
            ...
            attn_data.append(
                {
                    ...
                    'attn': [a[:, slice_a, slice_b].tolist() for a in attention],  # Convert numpy arrays to lists
                    'left_text': tokens[slice_a],
                    'right_text': tokens[slice_b]
                }
            )
            attn_data.append(
                {
                    ...
                    'attn': [a[:, slice_b, slice_a].tolist() for a in attention],  # Convert numpy arrays to lists
                    'left_text': tokens[slice_b],
                    'right_text': tokens[slice_a]
                }
            )
        ...
        if encoder_attention is not None:
            ...
            attn_data.append(
                {
                    'name': 'Encoder',
                    'attn': [a.tolist() for a in encoder_attention],  # Convert numpy arrays to lists
        ...
        if decoder_attention is not None:
            ...
            attn_data.append(
                {
                    'name': 'Decoder',
                    'attn': [a.tolist() for a in decoder_attention],  # Convert numpy arrays to lists
        ...
        if cross_attention is not None:
            ...
            attn_data.append(
                {
                    'name': 'Cross',
                    'attn': [a.tolist() for a in cross_attention],  # Convert numpy arrays to lists
                    'left_text': decoder_tokens,
                    'right_text': encoder_tokens
                }
            )
