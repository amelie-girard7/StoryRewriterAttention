    ...
    """Render model view
        ...
        Args:
            For self-attention models:
                attention: list of ``numpy.ndarray`` (one for each layer) of shape
                    ``(batch_size must be 1), num_heads, sequence_length, sequence_length)``  # <-- Updated type hint
                tokens: list of tokens
                sentence_b_start: index of first wordpiece in sentence B if input text is sentence pair (optional)
            For encoder-decoder models:
                encoder_attention: list of ``numpy.ndarray`` (one for each layer) of shape
                    ``(batch_size must be 1), num_heads, encoder_sequence_length, encoder_sequence_length)``  # <-- Updated type hint
                decoder_attention: list of ``numpy.ndarray`` (one for each layer) of shape
                    ``(batch_size must be 1), num_heads, decoder_sequence_length, decoder_sequence_length)``  # <-- Updated type hint
                cross_attention: list of ``numpy.ndarray`` (one for each layer) of shape
                    ``(batch_size must be 1), num_heads, decoder_sequence_length, encoder_sequence_length)``  # <-- Updated type hint
    ...
    if attention is not None:
        ...
        if sentence_b_start is None:
            attn_data.append(
                {
                    'name': None,
                    'attn': [att.tolist() for att in attention],  # <-- Converted numpy array to list
                    'left_text': tokens,
                    'right_text': tokens
                }
            )
            ...
            attn_data.append(
                {
                    ...
                    'attn': [att[:, :, slice_a, slice_a].tolist() for att in attention],  # <-- Converted numpy array to list
            ...
            attn_data.append(
                {
                    ...
                    'attn': [att[:, :, slice_b, slice_b].tolist() for att in attention],  # <-- Converted numpy array to list
            ...
            attn_data.append(
                {
                    ...
                    'attn': [att[:, :, slice_a, slice_b].tolist() for att in attention],  # <-- Converted numpy array to list
                    'left_text': tokens[slice_a],
                    'right_text': tokens[slice_b]
                }
            )
            attn_data.append(
                {
                    ...
                    'attn': [att[:, :, slice_b, slice_a].tolist() for att in attention],  # <-- Converted numpy array to list
                    'left_text': tokens[slice_b],
                    'right_text': tokens[slice_a]
                }
            )
        ...
        if encoder_attention is not None:
            ...
            attn_data.append(
                {
                    'name': 'Encoder',
                    'attn': [att.tolist() for att in encoder_attention],  # <-- Converted numpy array to list
        ...
        if decoder_attention is not None:
            ...
            attn_data.append(
                {
                    'name': 'Decoder',
                    'attn': [att.tolist() for att in decoder_attention],  # <-- Converted numpy array to list
        ...
        if cross_attention is not None:
            ...
            attn_data.append(
                {
                    'name': 'Cross',
                    'attn': [att.tolist() for att in cross_attention],  # <-- Converted numpy array to list
                    'left_text': decoder_tokens,
                    'right_text': encoder_tokens
                }
            )
